...
my$data = $ARGV[0] || 'http://spreadsheets.google.com/pub?key=rrP7dbsqMO8l4yFo9HXO1ZA&output=csv';
my$root = $ARGV[1] || "ApplyRoutes";
my$db;
...
foreach my$sname (keys %{$db->{sections}}) {
    ...
    for(my$i = 2; $i < @{$db->{codes}}; $i++) {
        ...
        (my$out = $sfile) =~ s/.resx$/.$db->{codes}[$i].resx/;
        print "Writing: $out\n";
    ...
    foreach my$row (@$rows) {
        ...
        if (@row) {
            if ($row[0] eq "CountryCode") {
                ...
                next;
            } elsif ($row[0] =~ m%^\w+/\w+\.resx$%) {
                $section = $row[0];
                next;
            }
            die "No section defined" unless $section;
            die "No country codes defined" unless $translations{codes};
            $translations{sections}{$section}{$row[0]} = \@row;
        }
    }
    return \%translations;
}
