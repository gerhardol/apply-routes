    ...
    class RouteManipulator : IAction
    {
        ...
        public void Run(Rectangle rectButton)
        {
            if (activities != null)
            {
                 ...
                 * */
                treePop.FitContent = true;
                treePop.Tree.Columns.Add(new TreeList.Column("Name", "Name", 100, StringAlignment.Near));
                treePop.Tree.RowData = Plugin.GetApplication().Logbook.Routes;
                treePop.ItemSelected += new TreeListPopup.ItemSelectedEventHandler(treePop_ItemSelected);
                treePop.PopupClosed += new EventHandler(treePop_PopupClosed);
                treePop.Activated += new EventHandler(treePop_PopupClosed);
                treePop.GotFocus += new EventHandler(treePop_PopupClosed);
